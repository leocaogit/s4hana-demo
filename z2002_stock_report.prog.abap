...
"定义ALV双击事件类
CLASS zcl_handle_events DEFINITION.
  PUBLIC SECTION.
...
CLASS zcl_handle_events IMPLEMENTATION.
...
FORM get_live_stock_info.
  ...
  IF gt_stock_id[] IS INITIAL.
    RETURN.
  ENDIF.
  CLEAR:it_res.
  REFRESH:it_res.
  LOOP AT gt_stock_id INTO gs_stock_id.
    CLEAR l_tmp.
    IF gs_stock_id-stock_id(1) < 6.
      CONCATENATE 'sz' gs_stock_id-stock_id INTO l_tmp.
    ENDIF.
    IF gs_stock_id-stock_id(1) = '6'.
      CONCATENATE 'sh' gs_stock_id-stock_id INTO l_tmp.
    ENDIF.
  ...
  ENDTRY.
  REFRESH gt_stock_pub_list .
  SPLIT l_str AT cl_abap_char_utilities=>newline
  INTO TABLE gt_stock_pub_list.
*  LOOP AT gt_stock_pub_list INTO l_str.
*    WRITE: / l_str.
*  ENDLOOP.
...
FORM fill_alv.
  ...
  LOOP AT gt_stock_pub_list INTO l_str.
    ...
    gs_stock-sid = l_str_field+13(6).
    gs_stock-sname = l_str_field+21.

    idx = 2.
    WHILE idx < 33.
      READ TABLE lt_res INDEX idx INTO l_str_field.
      idx = idx + 1.
      ASSIGN COMPONENT idx OF STRUCTURE gs_stock TO <fs_value>.
      <fs_value> = l_str_field.
    ENDWHILE.

    INSERT gs_stock INTO TABLE gt_stock.
  ENDLOOP.
  ...
  LOOP AT gt_stock ASSIGNING <fs_stock>.
...
FORM show_grid.
  ...
  TRY.
      ...
      "=====事件注册
      DATA: mo_handle_event TYPE REF TO zcl_handle_events.
